...
// ]
  ...
  const getCart = async () => {
    ...
    try{
      const data = (await Axios.get(`/api1/cart/${userId}`,config).then((res)=>res.data));
  ...
  const postCart = async (cartItem) => {
    ...
    const data = (await Axios.post("/api1/cart/create",requestData,config).then((res)=>res.data));
  ...
  const deleteCart = async (productId) =>{
    ...
    console.log(obj);
    console.log(config);
    const data = (await Axios.delete(`/api1/cart/delete/${userId}`,{headers: {
      Authorization: token
    },
    data: obj}).then((res)=>res.data));
  ...
  const updateCart = async (cartItem) =>{
    ...
    console.log("working like a magic man");
    const requestData = cartItem;
    console.log(userId);
    const data = (await Axios.put(`/api1/cart/update/${userId}`,requestData,config).then((res)=>res.data));
    ...
    setCart((prevCart) => 
    {
      ...
      const updatedCart = cart.map((item) => {
        if (item.productId === productId) {
          matchingItem = { ...item, quantity: item.quantity + cnt };
          return matchingItem;
        }
        return item;
      });
